...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'mobilenetv3_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
logger.info("Starting Neural Collapse analysis with MobileNetV3-Large on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
class MobileNetV3Custom(nn.Module):
    ...
    def get_layer_features(self, x, nc_layers=None):
        ...
        x = self.model.bn1(x)
        if not nc_layers or 'conv_stem' in nc_layers:
            features['conv_stem'] = x
        for i, block_group in enumerate(self.model.blocks, 0):
            for j, block in enumerate(block_group):
                x = block(x)
                if not nc_layers or f'block_{i}_{j}' in nc_layers:
                    features[f'block_{i}_{j}'] = x
        x = self.model.conv_head(x)
        x = self.model.act2(x)
        if not nc_layers or 'conv_head_act' in nc_layers:
            features['conv_head_act'] = x
    ...
    def get_penultimate_features(self, x):
        x = self.model.conv_stem(x)
        x = self.model.bn1(x)
        for block_group in self.model.blocks:
...
model_cifar10 = MobileNetV3Custom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = MobileNetV3Custom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = MobileNetV3Custom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        param.requires_grad = any(k in name for k in ['conv_stem', 'bn1', 'blocks', 'conv_head', 'act2', 'classifier'])
...
nc_layers = ['conv_stem', 'block_0_0', 'block_2_0', 'block_4_0', 'block_6_0', 'conv_head_act', 'global_pool']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
