...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'regnet_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with RegNet on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
# Define RegNet Model with custom feature extraction
class RegNetYCustom(nn.Module):
    def __init__(self, num_classes):
        super(RegNetYCustom, self).__init__()
        self.model = timm.create_model('regnety_040', pretrained=True, num_classes=num_classes)
        self.to(device)
    ...
    def get_layer_features(self, x, nc_layers=None):
        features = {}
        x = self.model.stem(x)
        if not nc_layers or 'stem' in nc_layers:
            features['stem'] = x
        x = self.model.s1(x)
        if not nc_layers or 's1' in nc_layers:
            features['s1'] = x
        x = self.model.s2(x)
        if not nc_layers or 's2' in nc_layers:
            features['s2'] = x
        x = self.model.s3(x)
        if not nc_layers or 's3' in nc_layers:
            features['s3'] = x
        x = self.model.s4(x)
        if not nc_layers or 's4' in nc_layers:
            features['s4'] = x
        x = self.model.head.drop(x) if self.model.head.drop else x
        x = self.model.head.global_pool(x)
        if not nc_layers or 'global_pool' in nc_layers:
            features['global_pool'] = x
        x = x.view(x.size(0), -1)
        if not nc_layers or 'penultimate' in nc_layers:
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if track_penultimate:
                with torch.no_grad():
                    ...
                    nc_loss = compute_nc_loss(features.get('penultimate', features['penultimate']), labels, num_classes, lambda_wc, lambda_etf, lambda_norm)
...
model_cifar10 = RegNetYCustom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = RegNetYCustom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = RegNetYCustom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        param.requires_grad = any(k in name for k in ['s1', 's2', 's3', 's4', 'head'])
...
nc_layers = ['stem', 's1', 's2', 's3', 's4', 'global_pool', 'penultimate']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
