...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'vit_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with ViT on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
# Define ViT Model with custom feature extraction
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if track_penultimate:
                with torch.no_grad():
                    ...
                    nc_loss = compute_nc_loss(features.get('penultimate', features['norm']), labels, num_classes, lambda_wc, lambda_etf, lambda_norm)
...
model_cifar10 = ViTCustom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = ViTCustom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = ViTCustom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        param.requires_grad = any(k in name for k in ['blocks', 'norm', 'head'])
...
nc_layers = ['patch_embed', 'blocks[0]', 'blocks[5]', 'blocks[10]', 'blocks[11]', 'norm', 'penultimate']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
