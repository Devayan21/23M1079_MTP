...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'shufflenetv2_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with ShuffleNetV2-x1.0 on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
class ShuffleNetV2Custom(nn.Module):
    ...
    def get_layer_features(self, x, nc_layers=None):
        ...
        if not nc_layers or 'conv1' in nc_layers:
            features['conv1'] = x
        x = self.model.maxpool(x)
        if not nc_layers or 'maxpool' in nc_layers:
            features['maxpool'] = x
        x = self.model.stage2(x)
        if not nc_layers or 'stage2' in nc_layers:
            features['stage2'] = x
        x = self.model.stage3(x)
        if not nc_layers or 'stage3' in nc_layers:
            features['stage3'] = x
        x = self.model.stage4(x)
        if not nc_layers or 'stage4' in nc_layers:
            features['stage4'] = x
        x = self.model.conv5(x)
        if not nc_layers or 'conv5' in nc_layers:
            features['conv5'] = x
        x = x.view(x.size(0), -1)
...
def compute_nc_loss(features, labels, n_classes, lambda_wc=1.0, lambda_etf=0.05, lambda_norm=0.005):
...
model_cifar10 = ShuffleNetV2Custom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = ShuffleNetV2Custom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = ShuffleNetV2Custom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        param.requires_grad = any(k in name for k in ['conv1', 'maxpool', 'stage2', 'stage3', 'stage4', 'conv5', 'fc'])
...
nc_layers = ['conv1', 'maxpool', 'stage2', 'stage3', 'stage4', 'conv5', 'penultimate']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
