...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'convnextbase_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with ConvNeXt-Base on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
class ConvNeXtBaseCustom(nn.Module):
    def __init__(self, num_classes):
        super(ConvNeXtBaseCustom, self).__init__()
...
# Training function with memory optimizations and all-batch tracking
...
model_cifar10 = ConvNeXtBaseCustom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = ConvNeXtBaseCustom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = ConvNeXtBaseCustom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        if any(k in name for k in ['stem', 'stages.0', 'stages.1', 'stages.2']):
            param.requires_grad = False
        else:
...
nc_layers = ['stem', 'stages[0]', 'stages[1]', 'stages[2]', 'stages[3]', 'norm', 'head.global_pool']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
