...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'resnet50_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with ResNet50 on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
# Define ResNet50 Model with custom feature extraction
class ResNet50Custom(nn.Module):
    def __init__(self, num_classes):
        super(ResNet50Custom, self).__init__()
        self.model = timm.create_model('resnet50', pretrained=True, num_classes=num_classes)
        trunc_normal_(self.model.fc.weight, std=0.02)
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            ...
            if track_penultimate:
                features = model.get_layer_features(inputs)
                all_features_penultimate.append(features['penultimate'].detach())
...
model_cifar10 = ResNet50Custom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = ResNet50Custom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = ResNet50Custom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        if not any(n in name for n in ["layer4", "global_pool", "fc"]):
            param.requires_grad = False
    optimizer_nc = optim.Adam(filter(lambda p: p.requires_grad, model_nc.parameters()), lr=0.001)

    nc_accuracy, nc_loss_value = train(
        model_nc, loader, optimizer_nc, criterion, num_classes, use_nc_loss=True, nc_layers=['penultimate'], nc_lambda=nc_lambda, experiment_name=f"Penultimate NC ({dataset_name})"
    )
...
logger.info("\n=== Part 3: NC Loss on Last Three Layers ===")
nc_layers = ['layer4', 'global_pool', 'penultimate']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
