...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'effnetv2s_nc_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger()
logger.info("Starting Neural Collapse analysis with EfficientNetV2-S on CIFAR-10, EuroSAT, Food-101, and DTD datasets.")
...
dtd_dataset = datasets.DTD(root='./ Bourdata', split='train', download=True, transform=transform)
dtd_loader = DataLoader(dtd_dataset, batch_size=32, shuffle=True)

# Define EfficientNetV2-S Model
class EfficientNetV2SCustom(nn.Module):
    ...
    def get_layer_features(self, x, nc_layers=None):
        ...
        if not nc_layers or 'stem' in nc_layers:
            features['stem'] = x
        for i, block in enumerate(self.model.features[1:-1], 1):
            x = block(x)
            if not nc_layers or f'block_{i}' in nc_layers:
                features[f'block_{i}'] = x
        x = self.model.features[-1](x)
        if not nc_layers or 'head_conv' in nc_layers:
            features['head_conv'] = x
        x = self.model.avgpool(x)
        if not nc_layers or 'head_pool' in nc_layers:
            features['head_pool'] = x
        return {k: v.to(device) for k, v in features.items()}

    def get_penultimate_features(self, x):
        x = self.model.features[0](x)
        for block in self.model.features[1:-1]:
            x = block(x)
        x = self.model.features[-1](x)
        x = self.model.avgpool(x)
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if track_penultimate:
                with torch.no_grad():
                    ...
                    nc_loss = compute_nc_loss(features.get('head_pool', features['head_pool']), labels, num_classes, lambda_wc, lambda_etf, lambda_norm)
...
model_cifar10 = EfficientNetV2SCustom(num_classes=10).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_baseline = EfficientNetV2SCustom(num_classes).to(device)
...
for dataset_name, (loader, num_classes) in datasets.items():
    ...
    model_nc = EfficientNetV2SCustom(num_classes).to(device)
    for name, param in model_nc.named_parameters():
        param.requires_grad = any(k in name for k in ['features', 'classifier'])
...
nc_layers = ['stem', 'block_2', 'block_4', 'block_6', 'head_conv', 'head_pool']
results_multi_layer = {}
for dataset_name, (loader, num_classes) in datasets.items():
